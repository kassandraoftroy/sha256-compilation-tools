...
def run_tests():
	...
	if vals == [True, True, True, True, True]:
		return "PASS"
	else:
		print vals
		return "FAIL"
...
def test_loop(x,y,z):
	...
	# Confirm Result
	return int("".join([str(i) for i in result]), 2) == check_loop_test(x,y,z)
...
def test_lsig(x,y,z):
	...
	# Confirm Result
	return int("".join([str(i) for i in result]), 2) == check_lsig_test(x,y,z)
...
def check_loop2_test(l):
	list1 = [int("".join([str(i) for i in l[224:256]]), 2), int("".join([str(i) for i in l[192:224]]), 2)]
